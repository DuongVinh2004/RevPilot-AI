...
resource "aws_security_group" "alb" {
  ...
  description = "Controls inbound public HTTPS traffic to API Gateway ALB"
  vpc_id      = aws_vpc.main.id

  ingress {
    description = "Public HTTPS ingress (TLS 1.3)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
...
resource "aws_security_group" "ecs_api" {
  ...
  ingress {
    ...
    to_port         = 8000
    protocol        = "tcp"
    security_groups = [] # Self-referencing resolved below via rule
  }

  egress {
    description = "Outbound HTTPS for AWS APIs and governed egress proxy"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
...
resource "aws_security_group" "ecs_workers" {
  ...
  egress {
    description = "Outbound HTTPS for governed model routing and AWS APIs"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [var.vpc_cidr]
