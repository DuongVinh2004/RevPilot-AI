# SPEC-ADR-006 — Decide retrieval baseline

TASK_ID: SPEC-ADR-006  
TITLE: Decide retrieval baseline  
...
DEPENDS_ON: SPEC-FND-003, SPEC-ADR-004, SPEC-ADR-005  
UNLOCKS: SPEC-FND-014  
PARALLEL_SAFE: YES — with ADR-007 after shared prerequisites.

## Objective and change boundary

Select MVP lexical/vector retrieval stores and explicit Qdrant/OpenSearch triggers. OUTPUT/WRITE/CREATE: `docs/31-adr/ADR-0006-retrieval-baseline.md`. READ: workload assumptions, PostgreSQL/tenancy ADRs, SRS evidence requirements. PROHIBITED WRITE: all other files.

## Decision envelope

INPUT FACTS: hybrid retrieval, ACL/effective-date filtering, early corpus scale, no measured need for separate stores. LOCKED: pgvector+PostgreSQL FTS baseline is proposed; Qdrant/OpenSearch are future options. RESOLVE: baseline, fusion/reranking boundary, benchmark and isolation triggers. MUST NOT REOPEN: need for hybrid retrieval/citation.

## Required content and controls

Select PostgreSQL FTS + pgvector with application-owned retrieval port and tenant filters enforced in data access. Compare Qdrant, OpenSearch, and both. Triggers include sustained p95 breach, OLTP interference, validated corpus ceiling, mandatory retrieval features, isolation/tiering, and operational ownership.

## Verification and acceptance

COMMAND: `rg -n "^# ADR-0006|PostgreSQL FTS|pgvector|Qdrant|OpenSearch|p95|OLTP|Revisit Triggers" docs/31-adr/ADR-0006-retrieval-baseline.md`. PASS only if now/later boundary and measurable triggers are explicit without installing new infrastructure.
